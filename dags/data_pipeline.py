...
def run_process(**context) -> None:
    """HDFS에서 가장 최근에 완성된 1시간 버킷의 데이터를 읽어 Cassandra에 저장한다.

    5분 전 시각을 기준으로 1시간 단위 내림(floor)을 적용한다.
    - 스케줄 실행(10:05) → 10:00 → floor → 10:00 버킷
    - 수동 실행(10:30)   → 10:25 → floor → 10:00 버킷
    """
    now_kst = pendulum.now(KST)
    ref_kst = now_kst.subtract(minutes=5)
    target_kst = ref_kst.replace(minute=0, second=0, microsecond=0)

    hdfs_path = mmsi_summary.get_hdfs_path_hour(target_kst)
...
def run_report(**context) -> None:
    """슬롯 검증 분석을 실행한다 (현재 시각 KST 기준 최근 1시간)."""
    now_kst = pendulum.now(KST)
    end_dt = now_kst
...
def run_rssi(**context) -> None:
    """RSSI/SNR 분석을 실행한다 (현재 시각 KST 기준 최근 1시간)."""
    now_kst = pendulum.now(KST)
    end_dt = now_kst
    start_dt = now_kst.subtract(hours=1)
